...
// Intervalo entre solicitudes (en milisegundos) para evitar abuso
unsigned long lastRequestTime = 0;
const unsigned long requestInterval = 10000;  // 10 segundos entre solicitudes
...
typedef struct Message {
  int valorHumedad;  // El mensaje debe coincidir con la estructura del emisor
} Message;
...
void loop() {
  ...
  if (millis() - lastRequestTime > requestInterval) {
    ...
    Serial.println(humedadIdeal);
    if (humedad < humedadIdeal && !rainInNext12Hours) {
      Serial.println("HUMEDAD INSUFICIENTE: LED ROJO Y BOMBA ACTIVA");
      ...
      Serial.println("HUMEDAD CORRECTA: LED VERDE Y BOMBA INACTIVA");
      digitalWrite(LED_ROJO, LOW);
